...
def _step6_classify_evidence(
    ...
    def classify_single_file(file_id, file_info):
        from ..clients.llm_client import call_llm_with_images, parse_json_response
        try:
            response = call_llm_with_images(llm, prompt_text, [file_info['presigned_url']])
    ...
    with ThreadPoolExecutor(max_workers=min(len(all_files), 5)) as executor:
        futures = {
            executor.submit(classify_single_file, fid, finfo): fid
...
def _step7_check_required_docs(
    ...
    missing_tx = [dt_id for dt_id in tx_doc_requires if dt_id not in tx_present_types]
    ...
    for rec_id, rec_fids in record_file_ids.items():
        ...
        missing_rec = [dt_id for dt_id in rec_doc_requires if dt_id not in rec_present_types]
        if missing_rec:
...
def _step8_match_evidence_to_data(
    ...
    # --- Record-level matching (threaded) ---
    active_rec_checks = {k: v for k, v in rec_checks.items() if v}
    if active_rec_checks and evidence_for_prompt and records:

        def match_single_record(record):
            try:
                # Get material name
                mat_name = ''
                if record.material_id:
                    mat = db.query(Material).filter(Material.id == record.material_id).first()
                    mat_name = (mat.name_en or mat.name_th or '') if mat else ''

                # Get destination name
                dest_name = ''
                if record.destination_id:
                    dest = db.query(UserLocation).filter(UserLocation.id == record.destination_id).first()
                    dest_name = (dest.name_en or '') if dest else ''

                # Build record evidence (tx images + this record's images)
                rec_evidence = []
                for ev in evidence_for_prompt:
                    rec_evidence.append({**ev, 'source': 'transaction'})
                ...
                prompt = build_record_matching_prompt(record_data, rec_evidence, active_rec_checks)
                response = call_llm_text_only(llm, prompt)
                token_usage['input_tokens'] += response.get('usage', {}).get('input_tokens', 0)
                token_usage['output_tokens'] += response.get('usage', {}).get('output_tokens', 0)
                return record.id, parse_json_response(response['content'])
            except Exception as e:
                logger.error(f"Record matching failed for record #{record.id}: {str(e)}")
                return record.id, {'error': str(e)}

        with ThreadPoolExecutor(max_workers=min(len(records), 5)) as executor:
            futures = [executor.submit(match_single_record, rec) for rec in records]
            for future in as_completed(futures):
                rec_id, match_result = future.result()
                result['record_matches'][rec_id] = match_result
...
def _step9_compose_and_save(
    ...
    try:
        ...
        if isinstance(tx_match, dict) and not tx_match.get('error'):
            for col, val in tx_match.items():
                if isinstance(val, dict) and val.get('match') is False:
                    has_mismatch = True
                    break
        for rec_id, rec_match in matching_result.get('record_matches', {}).items():
