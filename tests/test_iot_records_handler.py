...
class _AutoApproveDb(_FakeDb):
    """Answers the two column-only SELECTs the resolver makes."""

    def __init__(self, device_mode=None, org_flag=False):
        self.device_mode = device_mode
        self.org_flag = org_flag

    def execute(self, statement, params=None):
        sql = str(statement)
        value = self.device_mode if "iot_devices" in sql else self.org_flag
